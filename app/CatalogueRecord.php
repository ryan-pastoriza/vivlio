...
class CatalogueRecord extends Model
{
	...
	public function getOPAC($catalogue_id){
		...
		if($cat_rec->up_opac){
			...
			CatalogueRecord::where('catalogue_id',$catalogue_id)->update(['opac_info'=>$title.'_-OPAC-_'.$call_num.'_-OPAC-_'.$edition.'_-OPAC-_'.$isbnIssn.'_-OPAC-_'.$author.'_-OPAC-_'.$datePub.'_-OPAC-_'.$publisher.'_-OPAC-_'.$physDesc.'_-OPAC-_'.$copiesNum.'_-OPAC-_'.$copiesTotal.'_-OPAC-_'.$catalogue_id.'_-OPAC-_'.$matType.'_-OPAC-_'.$matType,'up_opac'=>0]);
	...
	public function getAccession($catalogue_id){
		...
		if($cat_rec->up_acc){
			...
			where('catalogue_id',$id)
				->update([
					'accession_info' => $isbnIssn.'_-ACCINFO-_'.$callNum.'_-ACCINFO-_'.$author.'_-ACCINFO-_'.$title.'_-ACCINFO-_'.$edition.'_-ACCINFO-_'.$volume.'_-ACCINFO-_'.$pages.'_-ACCINFO-_'.$price.'_-ACCINFO-_'.$copyAmt.'_-ACCINFO-_'.$publishingHouse.'_-ACCINFO-_'.$copyYear, 'up_acc' => 0
					]);
