							...
							<div class="row">
						  	...
						  	<div class="row">
						  		 ...
						  		 <div class="col-md-12" style="min-width:300px; overflow:auto;">
						  		 	 <table class="table table-striped table-hover" id="patron-table" style="">
						  		 	 	<thead>
						  		 	 		<tr>
						  		 	 			<th class="">#ID</th>
						  		 	 			<th class="">Patron</th>
						  		 	 			<th class="">Course</th>
						  		 	 			<th class="">Library Visit</th>
						  		 	 			<th class="">Borrowed Books</th>
						  		 	 			<th class="">Fines</th>
						  		 	 		</tr>
						  		 	 	</thead>
						  		 	 	<tbody>
	...
	<script type="text/javascript">
		$(document).ready(function(e){
			...
			$(document).on('change','#select-year',function(e){
				...
				var posting = $.post(url,{_token:"{{csrf_token()}}",yr:$(this).val(),mnt:"",category:"patron"});
					posting.done(function(data){
						createDataTable($('table#patron-table'),'Patron Information','fetch_patron',JSON.parse(data));
					});
			})
			$(document).on('change','#select-mnt',function(e){
				e.preventDefault();
				var url = "{{url('report/fetch_data_by_date')}}";
				var posting = $.post(url,{_token:"{{csrf_token()}}",yr:$('#select-year').val(),mnt:$(this).val(),category:"patron"});
					posting.done(function(data){
						createDataTable($('table#patron-table'),'LMU Information','fetch_patron',JSON.parse(data));
					});
			})
	   ...
	   {{--   function init_graph(){
		...
		function fetch_patron(){
			var url = "{{url('/report/fetch/patron')}}";
